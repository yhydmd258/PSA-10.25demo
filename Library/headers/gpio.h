...
#define XCONFIG_PIN_AS_GPIO(port,port_pin,mode)		(mode == 0) ? (port->PDDR &= ~((uint32_t)1 << port_pin)) : (port->PDDR |= (uint32_t)1 << port_pin)
...
#define XOUTPUT_SET(port,register_num)				port->PSOR |=(uint32_t)1<<register_num
...
#define XENABLE_INPUT(port,port_pin)				port->PIDR ^= (uint32_t)1<<port_pin
...
//	Fixing Function
#define GPIO_ENABLE_INPUT(port, port_pin)           XGPIO_ENABLE_INPUT(port, port_pin)
#define XGPIO_ENABLE_INPUT(port, port_pin)			(port->PIDR &= ~((uint32_t)1 << port_pin))

#define GPIO_DISABLE_INPUT(port,port_pin)			XGPIO_DISABLE_INPUT(port,port_pin)
#define XGPIO_DISABLE_INPUT(port,port_pin)			(port->PIDR |= ((uint32_t)1 << port_pin))

#define GPIO_READ_INPUT(port, port_pin)				XGPIO_READ_INPUT(port, port_pin)
#define XGPIO_READ_INPUT(port, port_pin)			((port->PDIR >> port_pin)&0x00000001)
